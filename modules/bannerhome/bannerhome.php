...
class BANNERHOME extends Module
{
	public function __construct()
	{
		$this->name = 'bannerhome';
		$this->description = 'M&oacute;dulo para banner sencillo';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Bannerhome_MONOLANGUAGE';
		$this->text_editor_content = 'Bannerhome_TEXTEDITOR';
		$this->content_wrapper = 'Bannerhome_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Bannerhome_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Bannerhome_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Banner home');
		$this->description = $this->l('M&oacute;dulo para banner sencillo');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !BANNERHOMEModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = BANNERHOMEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = BANNERHOMEModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					BANNERHOMEModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					BANNERHOMEModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																BANNERHOMEModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class BANNERHOMEModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( BANNERHOMEModel::createFilesTable()
				&& BANNERHOMEModel::createContentTable());
	}
