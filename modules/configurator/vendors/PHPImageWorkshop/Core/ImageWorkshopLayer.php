...
class ImageWorkshopLayer
{
    ...
    public function __construct($image, array $exif = array())
    {
        if (!extension_loaded('gd')) {
            throw new ImageWorkshopLayerException('PHPImageWorkshop requires the GD extension to be loaded.',
                static::ERROR_GD_NOT_INSTALLED);
        }

        if (gettype($image) != 'resource' && gettype($image) != '\resource') {
            throw new ImageWorkshopLayerException('You must give a php image var to initialize a layer.',
                static::ERROR_PHP_IMAGE_VAR_NOT_USED);
        }
    ...
    public function mergeDown($layerId)
    {
        ...
        if ($this->isLayerInIndex($layerId)) {
            ...
            if ($layerLevel > 1) {
                ...
                $layerTmp->addLayer(1, $underLayer, $underLayerPositionX - $minLayerPositionX,
                    $underLayerPositionY - $minLayerPositionY);
                $layerTmp->addLayer(2, $layer, $layerPositionX - $minLayerPositionX,
                    $layerPositionY - $minLayerPositionY);
     ...
     * $positionX, $positionY, $position can be ignored unless you choose a new width AND a new height AND to conserve proportion.
     */
    public function resizeInPercent(
        $percentWidth = null,
        $percentHeight = null,
        $converseProportion = false,
        $positionX = 0,
        ...
        $this->resize(self::UNIT_PERCENT, $percentWidth, $percentHeight, $converseProportion, $positionX, $positionY,
            $position);
    }
    ...
    public function crop(
        ...
        if ($width < 0 || $height < 0) {
            throw new ImageWorkshopLayerException('You can\'t use negative $width or $height for "' . __METHOD__ . '" method.',
                static::ERROR_NEGATIVE_NUMBER_USED);
        }
     ...
     * @param string $position
     */
    public function cropToAspectRatioInPercent(
        $width = 0,
        $height = 0,
        $positionXPercent = 0,
        $positionYPercent = 0,
     ...
     * @param string $position
     */
    public function cropToAspectRatio(
        $unit = self::UNIT_PIXEL,
        $width = 0,
        $height = 0,
        $positionX = 0,
    ...
    public function rotate($degrees)
    {
        if ($degrees != 0) {
            ...
            $rotationDegrees = ($degrees > 0) ? (int)(360 * (1 - $degrees / 360)) : $degrees; // Used to fixed PHP >= 5.5 rotation with base angle 90°, 180°
    ...
    public function opacity($opacity, $recursive = true)
    {
        ...
        ImageWorkshopLib::imageCopyMergeAlpha($transparentImage, $this->image, 0, 0, 0, 0, $this->getWidth(),
            $this->getHeight(), $opacity);
    ...
    public function flip($type = 'horizontal')
    {
        ...
        if ($type == 'horizontal') {
            ...
            imagecopyresampled($temp, $this->image, 0, 0, $this->width - 1, 0, $this->width, $this->height,
                -$this->width, $this->height);
            $this->image = $temp;

            foreach ($this->layerPositions as $layerId => $layerPositions) {

                $this->changePosition($layerId,
                    $this->width - $this->layers[$layerId]->getWidth() - $layerPositions['x'], $layerPositions['y']);
            }

        } elseif ($type == 'vertical') {

            imagecopyresampled($temp, $this->image, 0, 0, 0, $this->height - 1, $this->width, $this->height,
                $this->width, -$this->height);
            $this->image = $temp;

            foreach ($this->layerPositions as $layerId => $layerPositions) {

                $this->changePosition($layerId, $layerPositions['x'],
                    $this->height - $this->layers[$layerId]->getHeight() - $layerPositions['y']);
            }
        }
    ...
    public function write(
        ...
        if (!file_exists($fontPath)) {
            throw new ImageWorkshopLayerException('Can\'t find a font file at this path : "' . $fontPath . '".',
                static::ERROR_FONT_NOT_FOUND);
        }

        $RGBTextColor = ImageWorkshopLib::convertHexToRGB($color);
        $textColor = imagecolorallocate($this->image, $RGBTextColor['R'], $RGBTextColor['G'], $RGBTextColor['B']);

        return imagettftext($this->image, $fontSize, $fontRotation, $positionX, $positionY, $textColor, $fontPath,
            $text);
    }
    ...
    public function getResult($backgroundColor = null)
    {
        ...
        foreach ($this->layerLevels as $layerLevel => $layerId) {
            ...
            if ($this->layerPositions[$layerId]['x'] != 0 || $this->layerPositions[$layerId]['y'] != 0) {
                ...
                ImageWorkshopLib::mergeTwoImages($virginLayoutImageTmp, $imagesToMerge[$layerLevel],
                    $this->layerPositions[$layerId]['x'], $this->layerPositions[$layerId]['y'], 0, 0);
    ...
    public function save(
        ...
        if (is_file($folder)) {
            throw new ImageWorkshopLayerException(sprintf('Destination folder "%s" is a file.', $folder),
                self::ERROR_NOT_WRITABLE_FOLDER);
        }

        if ((!is_dir($folder) && !$createFolders)) {
            throw new ImageWorkshopLayerException(sprintf('Destination folder "%s" not exists.', $folder),
                self::ERROR_NOT_WRITABLE_FOLDER);
        }

        if (is_dir($folder) && !is_writable($folder)) {
            throw new ImageWorkshopLayerException(sprintf('Destination folder "%s" not writable.', $folder),
                self::ERROR_NOT_WRITABLE_FOLDER);
        }
        ...
        if (!is_dir($folder) && $createFolders) {
            if (!mkdir($folder, 0777, true)) {
                throw new ImageWorkshopLayerException(sprintf('Unable to create destination folder "%s".', $folder),
                    self::ERROR_NOT_WRITABLE_FOLDER);
            }
        ...
        if ($extension == 'jpg' || $extension == 'jpeg') {
            ...
            $isSaved = imagepng($image, $filename, (int)$imageQuality);

        } else {

            throw new ImageWorkshopLayerException(sprintf('Image format "%s" not supported.', $extension),
                self::ERROR_NOT_SUPPORTED_FORMAT);

        }

        if (!$isSaved) {
            throw new ImageWorkshopLayerException(sprintf('Error occurs when save image "%s".', $folder),
                self::ERROR_UNKNOW);
        }
    ...
    protected function indexLayer($layerLevel, $layer, $positionX = 0, $positionY = 0, $position)
    {
        ...
        $this->layerPositions[$layerId] = ImageWorkshopLib::calculatePositions($this->getWidth(), $this->getHeight(),
            $layer->getWidth(), $layer->getHeight(), $positionX, $positionY, $position);
    ...
    public function resizeBackground($newWidth, $newHeight)
    {
        ...
        imagecopyresampled($virginLayoutImage, $this->image, 0, 0, 0, 0, $this->width, $this->height, $oldWidth,
            $oldHeight);
     ...
     * @deprecated
     */
    public function resizeInPourcent(
        $percentWidth = null,
        $percentHeight = null,
        $converseProportion = false,
        $positionX = 0,
        ...
        throw new ImageWorkshopLayerException('Method resizeInPourcent() was renamed resizeInPercent(). Use resizeInPercent() instead.',
            static::METHOD_DEPRECATED);
    }
    ...
    {
        throw new ImageWorkshopLayerException('Method resizeByLargestSideInPourcent() was renamed resizeByLargestSideInPercent(). Use resizeByLargestSideInPercent() instead.',
            static::METHOD_DEPRECATED);
    }
    ...
    {
        throw new ImageWorkshopLayerException('Method resizeByNarrowSideInPourcent() was renamed resizeByNarrowSideInPercent(). Use resizeByNarrowSideInPercent() instead.',
            static::METHOD_DEPRECATED);
    }
     ...
     * @deprecated
     */
    public function cropInPourcent(
        $percentWidth = 0,
        $percentHeight = 0,
        $positionXPercent = 0,
        $positionYPercent = 0,
        ...
        throw new ImageWorkshopLayerException('Method cropInPourcent() was renamed cropInPercent(). Use cropInPercent() instead.',
            static::METHOD_DEPRECATED);
    }
    ...
    {
        throw new ImageWorkshopLayerException('Method cropMaximumInPourcent() was renamed cropMaximumInPercent(). Use cropMaximumInPercent() instead.',
            static::METHOD_DEPRECATED);
    }
}
