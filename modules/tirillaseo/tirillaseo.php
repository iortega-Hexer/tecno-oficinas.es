...
class TIRILLASEO extends Module
{
	public function __construct()
	{
		$this->name = 'tirillaseo';
		$this->description = 'M&oacute;dulo para texto seo de la home';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Tirillaseo_MONOLANGUAGE';
		$this->text_editor_content = 'Tirillaseo_TEXTEDITOR';
		$this->content_wrapper = 'Tirillaseo_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Tirillaseo_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Tirillaseo_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Tirilla SEO');
		$this->description = $this->l('M&oacute;dulo para texto seo de la home');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !TIRILLASEOModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = TIRILLASEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = TIRILLASEOModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					TIRILLASEOModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					TIRILLASEOModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																TIRILLASEOModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class TIRILLASEOModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( TIRILLASEOModel::createFilesTable()
				&& TIRILLASEOModel::createContentTable());
	}
