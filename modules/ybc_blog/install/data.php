...
if (!defined('_PS_VERSION_'))
    ...
    for ($i = 1; $i <= 1; $i++){
        ...
        $req ="INSERT INTO "._DB_PREFIX_."ybc_blog_post_category(id_post, id_category,position)  VALUES(".(int)$i.",1,".(int)$i.")";
        Db::getInstance()->execute($req);
        
        foreach($languages as $language)
        {
            $req ="INSERT INTO "._DB_PREFIX_."ybc_blog_tag(id_post, id_lang, tag, click_number)  VALUES(".(int)$i.",".(int)$language['id_lang'].",'Lorem',0)";
            Db::getInstance()->execute($req);
            $req ="INSERT INTO "._DB_PREFIX_."ybc_blog_tag(id_post, id_lang, tag, click_number)  VALUES(".(int)$i.",".(int)$language['id_lang'].",'Consectetur',0)";
            Db::getInstance()->execute($req);
        }    
    }
    $captions=array(
