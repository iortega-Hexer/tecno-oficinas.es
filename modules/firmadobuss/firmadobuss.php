...
class FIRMADOBUSS extends Module
{
	public function __construct()
	{
		$this->name = 'firmadobuss';
		$this->description = 'Firma de Dobuss';
		$this->tab = 'front_office_features';
		$this->version = '1.1.3';
		$this->author = 'emotionLoop';
		$this->need_instance = 0;
		$this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.7');
		$this->monolanguage_content = 'Firmadobuss_MONOLANGUAGE';
		$this->text_editor_content = 'Firmadobuss_TEXTEDITOR';
		$this->content_wrapper = 'Firmadobuss_CONTENTWRAPPER';
		$this->content_wrapper_class = 'Firmadobuss_CONTENTWRAPPER_CLASS';
		$this->content_wrapper_id = 'Firmadobuss_CONTENTWRAPPER_ID';
		$this->bootstrap = true;
		$this->_html = '';
		...
		$this->displayName = $this->l('Firma Dobuss');
		$this->description = $this->l('Firma de Dobuss');
	...
	public function install()
	{
		...
		if (!parent::install() || !$this->registerHook('header') || !$this->registerHook('footer') || !FIRMADOBUSSModel::createTables())
			return false;
	...
	public function addFilesToTemplate($path = null)
	{
		...
		$files_data = FIRMADOBUSSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id );
	...
	{
		$content_query = FIRMADOBUSSModel::getContent( $this->selected_store_id, $this->selected_language_id );
		$pre_content = '';
		$pos_content = '';
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
				...
				&& (int)Configuration::get('PS_LANG_DEFAULT') == $this->selected_language_id ))
			{
				...
				if (Tools::getValue('content_text') !== false && $this->ignore_changes_content_changes == false)
					FIRMADOBUSSModel::setContent( Tools::getValue('content_text'), $this->selected_store_id, $this->selected_language_id );

				//store the files to be used
				if (Tools::getValue('headerFiles') !== false)
					FIRMADOBUSSModel::setFiles(
	...
	public function displayForm()
	{
		...
		$helper->fields_value['headerFiles[]'] = $this->processSelectFilesForMultiselect(
																FIRMADOBUSSModel::getFilesInUse( $this->selected_store_id, $this->selected_language_id )
																);
...
class FIRMADOBUSSModel extends ObjectModel
{
	...
	public static function createTables()
	{
		...
		return ( FIRMADOBUSSModel::createFilesTable()
				&& FIRMADOBUSSModel::createContentTable());
	}
