    ...
    {if $nodes|count}
      <ul class="category-sub-menu">
        {foreach from=$nodes item=node}
          <li data-depth="{$depth}">
            {if $depth===0}
              <a href="{$node.link}" class="border-none">
                <div class="fondo-imagen">
                  {if $node.miniatura}
                    <img src="/img/c/{$node.id}_thumb.jpg" class="imgm"/>
                  {else}
                    <img src="/img/c/es-default-categorias_tree.jpg" class="imgm"/>
                  {/if}
                </div>
                <div class="btn-hover">
                  {l s='Ver categoría' mod='ps_categorytree'}
                </div>
              </a>
              ...
              <div class="cat-description">
                {$node.desc|strip_tags:false|truncate:110:'...'}
              </div>
              {if $node.children}
