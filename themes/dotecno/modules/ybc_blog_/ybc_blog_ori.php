                ...
                'enabled' => array(
                    ...
                    'filter_list' => array(
                        ...
                        'list' => array(
                            ...
                            2 => array(
                                'enabled' => 0,
                                'title' => $this->l('Draft/Unpublished')
                            ),
                            3=>array(
                                'enabled'=>2,
                                'title' =>$this->l('Schedule publish date')
 ...
 */
 ...
 public function renderConfig($configs,$title,$icon)
 {
    ...
    $position_sidebar= explode(',',Configuration::get('YBC_BLOG_POSITION_SIDEBAR') ? Configuration::get('YBC_BLOG_POSITION_SIDEBAR'):'sidebar_categories,sidebar_search,sidebar_new,sidebar_popular,sidebar_featured,sidebar_tags,sidebar_gallery,sidebar_archived,sidebar_comments,sidebar_authors,sidebar_rss');
...
//            $errors[] = $this->l('Number of category per page need to be greater than 0');
 ...
 public function hookDisplayLeftColumn()
 {
    ...
    $sidebars_postion= explode(',',Configuration::get('YBC_BLOG_POSITION_SIDEBAR') ? Configuration::get('YBC_BLOG_POSITION_SIDEBAR') :'sidebar_categories,sidebar_search,sidebar_new,sidebar_popular,sidebar_featured,sidebar_tags,sidebar_gallery,sidebar_archived,sidebar_comments,sidebar_authors,sidebar_rss');
    $this->context->smarty->assign(
        array(
...
//        die('x');
    ...
    public function setMetas()
    {
        ...
        {
            $employee = $this->getAuthorById(Tools::getValue('id_author'),Tools::getValue('is_customer'));                
...
//            	$excludeIds = '';
        ...
        if ($items && $acc)
        	foreach ($items AS $item)
        		echo trim(str_replace('|','',$item['name'])).(!empty($item['reference']) ? ' (ref: '.$item['reference'].')' : '').'|'.(int)($item['id_product']).'|'.str_replace('http://', Tools::getShopProtocol(), Context::getContext()->link->getImageLink($item['link_rewrite'], $item['id_image'], $type_image))."\n";
        elseif ($items)
        {
        	// packs
        	$results = array();
   ...
   {        
    ...
    public function hookDisplayFooterProduct($params)
    {
        ...
        LEFT JOIN "._DB_PREFIX_."ybc_blog_post_lang pl ON (p.id_post=pl.id_post)
        WHERE FIND_IN_SET('".(int)Tools::getValue('id_product')."', REPLACE(products,'-', ',')) AND pl.id_lang=".(int)$this->context->language->id.' LIMIT 0,'.(int)$limit;
        $posts= Db::getInstance()->executeS($sql);
        if($posts)
        {
   ...
   public function displayBlogCategoryTre($blockCategTree,$selected_categories,$name='',$disabled_categories=array())
    ...
    public function displayPostRelatedCategories($id_category)
    {
        if(!Configuration::get('YBC_BLOG_DISPLAY_CATEGORY_PAGE') || !Configuration::get('YBC_BLOG_NUMBER_POST_IN_CATEGORY'))
            return '';         
        $posts= $this->getPostsWithFilter(' AND rpc.id_category='.(int)$id_category,$this->sort,0,Configuration::get('YBC_BLOG_NUMBER_POST_IN_CATEGORY',8));
        if($posts)
            foreach($posts as &$post)
            {
                $post['link'] = $this->getLink('blog',array('id_post' => $post['id_post']));
