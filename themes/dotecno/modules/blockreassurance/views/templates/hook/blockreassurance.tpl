...
{if $elements}
    <div class="container container-blockreassurance">
      <div class="blockreassurance">
        {assign var=numCols value=$elements|@count}
          ...
          {foreach from=$elements item=block key=$key name=blocks}
            ...
            <div class="col-md-{$sizeCol_md} {$offsetCol_md} col-sm-{$sizeCol_sm} {$offsetCol_sm} col-xs-12"
              ...
              <div class="block-icon">
                {if $block.image}
                <img src="{$block.image}" title="{$block.text|escape:'quotes'}" alt="{$block.text|escape:'quotes'}">
                {/if}
              </div>
              <div class="block-title" style="color:{$psr_text_color}">
